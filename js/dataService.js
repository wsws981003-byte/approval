...
class DataService {
    ...
    async getApprovals() {
        if (this.storageType === 'supabase' && this.supabase) {
            ...
            // Supabase 필드명(snake_case)을 JavaScript 필드명(camelCase)으로 변환
    ...
    async getMonthlyApprovals(year, month) {
        ...
        return allApprovals.filter(approval => {
            const approvalDate = new Date(approval.created_at || approval.createdAt);
    ...
    async saveApproval(approval) {
        if (this.storageType === 'supabase' && this.supabase) {
            // site_id가 Supabase에 존재하는지 확인
            // site_name으로 site를 찾아서 실제 Supabase id를 사용
            ...
            if (approval.siteName) {
                ...
                if (matchingSite) {
                    ...
                    // site_name으로 찾을 수 없으면 site_id로 시도
                    if (approval.siteId) {
                        const siteById = allSites.find(s => s.id === approval.siteId || s.id === parseInt(approval.siteId));
                        if (siteById) {
                            supabaseSiteId = siteById.id;
                        }
                    }
                    if (!supabaseSiteId) {
                        console.warn(`Site "${approval.siteName}" (ID: ${approval.siteId}) not found in Supabase, using null for site_id`);
                    }
                }
            }
            ...
            const supabaseData = {
                ...
                title: approval.title,
                site_id: supabaseSiteId, // Supabase에 존재하는 site_id만 사용, 없으면 null
    ...
    async updateApprovedUser(username, updates) {
        if (this.storageType === 'supabase' && this.supabase) {
            // JavaScript 필드명(camelCase)을 Supabase 필드명(snake_case)으로 변환
            const supabaseUpdates = {};
...
// Supabase 연동 시 사용 예시 (주석)
/*
// Supabase 초기화 후
import { createClient } from '@supabase/supabase-js';

const supabase = createClient('YOUR_SUPABASE_URL', 'YOUR_SUPABASE_ANON_KEY');
dataService.setSupabase(supabase);

// 이제 백업 기능이 자동으로 Supabase에서 데이터를 가져옵니다!
// createFullBackup(), createMonthlyBackup() 함수가 자동으로 Supabase 데이터를 사용합니다.
*/
